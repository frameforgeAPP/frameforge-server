...
export default function ThemeSelector({ currentTheme, onSelectTheme, onClose, currentBackground, onSelectBackground, onUploadBackground, globalSettings, onUpdateGlobalSettings, isDarkMode, onToggleDarkMode, onOpenClock, customColors, onUpdateCustomColor, hardwareLabels, onUpdateHardwareLabel }) {
    ...
    useEffect(() => {
        ...
        const proStatus = PremiumManager.isPro();
        if (proStatus) {
            setIsPro(true);
        }
        // Get Device ID
        // Fetch Remote Premium Configuration
        ConfigService.getPremiumConfiguration().then(config => {
            setPremiumThemes(config.premiumThemes);
            setPremiumBackgrounds(config.premiumBackgrounds);
                            ...
                            <button
                                ...
                                onClick={() => {
                                    ...
                                    if (bg.id === 'custom') {
                                        ...
                                        logEvent(analytics, 'select_background', { background_name: bg.id });
