            ...
            <div className="bg-gray-900 border border-gray-800 rounded-2xl max-w-md w-full max-h-[85vh] overflow-hidden flex flex-col relative">
                    ...
                    <div className="absolute inset-0 z-10 bg-black/60 backdrop-blur-[2px] flex flex-col items-center justify-center p-6 text-center">
                        <div className="bg-gray-900 border border-yellow-500/30 p-6 rounded-2xl shadow-2xl max-w-xs w-full">
                            ...
                            <p className="text-sm text-gray-400 mb-6">
                                O sistema de alertas avançado é exclusivo para usuários PRO.
                            </p>
                            <button
                                onClick={() => setShowUnlockModal(true)}
                                className="w-full py-3 bg-yellow-500 hover:bg-yellow-600 text-black font-bold rounded-xl transition-colors flex items-center justify-center gap-2"
                            >
                                <Unlock size={18} />
                                Desbloquear Agora
                            </button>
                ...
                <div className="flex items-center justify-between p-6 border-b border-gray-800">
                    <div className="flex items-center gap-3">
                        ...
                        {isPro && <span className="text-xs bg-yellow-500/20 text-yellow-400 px-2 py-0.5 rounded border border-yellow-500/30 font-bold">PRO</span>}
                    </div>
                    ...
                    <div className={`space-y-4 ${!settings.enabled ? 'opacity-50 pointer-events-none' : ''}`}>
                        ...
                        <div className="p-4 bg-gray-800/30 rounded-xl border border-gray-700">
                            <div className="flex items-center justify-between mb-3">
                                ...
                                {/* Individual Toggle */}
                                <button
                                    onClick={() => updateSetting('cpuAlertEnabled', !settings.cpuAlertEnabled)}
                                    className={`w-10 h-5 rounded-full transition-colors relative ${settings.cpuAlertEnabled !== false ? 'bg-orange-500' : 'bg-gray-600'}`}
                                >
                                    <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-transform ${settings.cpuAlertEnabled !== false ? 'translate-x-6' : 'translate-x-1'}`} />
                                </button>
                            </div>
                            <div className={`flex items-center gap-4 ${settings.cpuAlertEnabled === false ? 'opacity-50 pointer-events-none' : ''}`}>
                                <input
                                    type="range"
                                    min="60"
                                    max="100"
                        ...
                        <div className="p-4 bg-gray-800/30 rounded-xl border border-gray-700">
                            <div className="flex items-center justify-between mb-3">
                                ...
                                {/* Individual Toggle */}
                                <button
                                    onClick={() => updateSetting('gpuAlertEnabled', !settings.gpuAlertEnabled)}
                                    className={`w-10 h-5 rounded-full transition-colors relative ${settings.gpuAlertEnabled !== false ? 'bg-green-500' : 'bg-gray-600'}`}
                                >
                                    <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-transform ${settings.gpuAlertEnabled !== false ? 'translate-x-6' : 'translate-x-1'}`} />
                                </button>
                            </div>
                            <div className={`flex items-center gap-4 ${settings.gpuAlertEnabled === false ? 'opacity-50 pointer-events-none' : ''}`}>
                                <input
                                    type="range"
                                    min="60"
                                    max="100"
                        ...
                        <div className="p-4 bg-gray-800/30 rounded-xl border border-gray-700">
                            <div className="flex items-center justify-between mb-3">
                                ...
                                {/* Individual Toggle */}
                                <button
                                    onClick={() => updateSetting('fpsAlertEnabled', !settings.fpsAlertEnabled)}
                                    className={`w-10 h-5 rounded-full transition-colors relative ${settings.fpsAlertEnabled !== false ? 'bg-red-500' : 'bg-gray-600'}`}
                                >
                                    <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-transform ${settings.fpsAlertEnabled !== false ? 'translate-x-6' : 'translate-x-1'}`} />
                                </button>
                            </div>
